...
export const MobileStockChart: FC<StockChartProps> = ({ data }) => {
  ...
  const option = useMemo(() => {
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          if (candle) {
            ...
            result += `<div style="font-weight: bold; margin-bottom: 4px; font-size: 11px;">${date}</div>`;
            result += `
            ...
            result += `<div style="font-weight: bold; margin-bottom: 4px; font-size: 11px;">${params[0].name}</div>`;
          }
          ...
          params.forEach((p: any) => {
            if (p.seriesName !== 'Price') {
              ...
              if (typeof val === 'number') {
                if (p.seriesName === 'Volume') formattedVal = Math.round(val).toLocaleString();
                else formattedVal = val.toLocaleString(undefined, { maximumFractionDigits: 2 });
              }

              result += `
              <div style="display: flex; justify-content: space-between; gap: 10px; align-items: center; font-size: 10px;">
      ...
      axisPointer: {
        link: [{ xAxisIndex: 'all' }],
        label: { backgroundColor: tokens.background[3], color: tokens.foreground[1], fontSize: 10 },
      },
      grid: grids,
      xAxis: xAxes,
      yAxis: yAxes,
      dataZoom: [
        { type: 'inside', xAxisIndex: xAxisIndices, start: 70, end: 100 },
        {
          ...
          borderColor: tokens.background[3],
          textStyle: { color: tokens.foreground[3], fontSize: 9 },
          handleStyle: { color: tokens.background[3] },
        },
      ],
      series,
    };
  }, [chartData.dates, series, activeOverlays, mode, tokens]);
